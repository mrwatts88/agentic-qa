...
export interface Adapter {
  tool: string;
  /**
   * Seconds rather than milliseconds per run. Left out of the commit hook by
   * default, so every commit stays quick; its corpus rules are still enforced
   * at the turn boundary and in CI.
   */
  slow?: boolean;
